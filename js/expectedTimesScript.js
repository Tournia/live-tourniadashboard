function calculateExpectedTime(um, my_PoolName, poolPropertiesObject, singleUMData, nrOfCourts, my_statusText, playersNotReady, playersPlaying, playerNamesCurrentlyPlayingArray, playersPlayingObects, predictedTimeLeftofPlayingPlayersArray) {
	try{
							...
							if(prevShiftNr == 1){
								...
								if(Playing == true){
									...
									if(ppUpcomingMatchInfoObjects.length > 0){
										...
										log("length ppU:", ppUpcomingMatchInfoObjects.length)
										ppUpcomingMatchInfoObjects.shift()
									} else {
										log("shift 2 match follows regular pattern after playing matches")
										log("rtplength", rtpUpcomingMatchInfoObjects.length)
										//if(rtpUpcomingMatchInfoObjects.length != 0){
											//log("in rtp")
										...
										//} else {
											//log("pplength:", ppUpcomingMatchInfoObjects)
		...
		//show shiftNr in table if runnning runLocal
		if(runLocal == true){
			if(secondpart == true){
				if(endShift == true){
					my_shiftNr = shiftNr + ". "
				} else {
					my_shiftNr = (shiftNr + 1) + ". "
				}
		...
		if (inUnavailable == true) {
			...
			singleUMData.shiftNrExpectedTimeMinsStdDev = my_shiftNr + "unknown"
			...
			if (freeCourt == true) {
				...
				singleUMData.shiftNrExpectedTimeMinsStdDev = my_shiftNr + "free court available"
				...
				singleUMData.shiftNrExpectedTimeMinsStdDev = my_shiftNr + finalStdDeviationTime + " mins." + stars
		...
		singleUMData.expectedTimeSecs = "unknown"
		singleUMData.shiftNrExpectedTimeSecs = my_shiftNr + "unknown"
		singleUMData.expectedTimeMins = "unknown"
		singleUMData.shiftNrExpectedTimeSecsMins = my_shiftNr + "unknown"
		singleUMData.shiftNrExpectedTimeMinsStdDev = my_shiftNr + "unknown"
		singleUMData.NamedExpectedTime = "unknown"
		singleUMData.StdDevExpectedTime = "unknown"
...
function adjustExpectedTimes(){
	rtpObjects = []
	var rtpExpectedTimesArray = []
	var rtpMatchIdsArray = []
	for(var o = 0; o < allUMdata.length; o++) {
		if(allUMdata[o].readyToPlay == true || allUMdata[o].shiftNr > 2){//(allUMdata[o].expectedTimeSecs > (2*(maxAvgTime + timeBetweenMatches)) && allUMdata[o].unavailablePlayers == false)){
	...
	for (var objNr = 0; objNr < rtpObjects.length; objNr++){
		...
		if(rtpObjects[objNr].expectedTimeSecs == 0){
			...
			rtpObjects[objNr].shiftNrExpectedTimeMinsStdDev_ad = rtpObjects[objNr].sNr + "free court available"
		...
		/*} else if( isNaN(rtpObjects[objNr].expectedTimeSecs) == true || isNaN(rtpObjects[objNr].expectedTimeMins) == true) { 
			rtpObjects[objNr].shiftNrExpectedTimeSecsMins_ad = rtpObjects[objNr].sNr + "unknown?"
			rtpObjects[objNr].shiftNrExpectedTimeSecs_ad = rtpObjects[objNr].sNr + "unknown?"
			rtpObjects[objNr].shiftNrExpectedTimeMinsStdDev_ad = rtpObjects[objNr].sNr + "unknown?"
			rtpObjects[objNr].stdDevTime_ad = "unknown"
			rtpObjects[objNr].NamedExpectedTime_ad = "unknown?"
			rtpObjects[objNr].StdDevExpectedTime_ad = "unknown?"*/
			...
			var extraExpectedTimeNames = namedExpectedTime(rtpObjects[objNr].poolName, Math.ceil(rtpObjects[objNr].expectedTimeSecs_ad/60), rtpObjects[objNr].stdDevTime) 
			...
			rtpObjects[objNr].shiftNrExpectedTimeMinsStdDev_ad = extraExpectedTimeNames[1] + " mins." + rtpObjects[objNr].stars
			rtpObjects[objNr].stdDevTime_ad = rtpObjects[objNr].stdDevTime	
		}
		...
		singleUMObject.shiftNrExpectedTimeMinsStdDev = rtpObjects[objNr].shiftNrExpectedTimeMinsStdDev_ad
		...
		//find in allUpcomingMatchInfoObjects and replace values
		var singleUpcomingMatchObject = allUpcomingMatchInfoObjects.find(x => x.localId === my_matchID)
		singleUpcomingMatchObject.expectedTime = rtpObjects[objNr].expectedTimeSecs_ad
		/*if( isNaN(singleUMObject.expectedTimeSecs_ad) == true){
			singleUMObject.expectedTimeSecs = "unknown?"
			singleUMObject.expectedTimeMins = "unknown?"
			singleUMObject.shiftNrExpectedTimeSecsMins = "unknown?"
			singleUMObject.shiftNrExpectedTimeSecs = "unknown?"
			singleUMObject.shiftNrExpectedTimeMinsStdDev = "unknown?"
			singleUMObject.NamedExpectedTime = "unknown?"
			singleUMObject.stdDevTime = "unknown?"
			singleUMObject.StdDevExpectedTime = "unknown?"
		}
		*/
		
		//Adjust Database Strings
		var ifMatchAdjusted = inArray(my_matchID, adjustedStringsArray) 
		if(ifMatchAdjusted == false){
			for(str in UM_startLog){
				if(UM_startLog[str].includes(";"+my_matchID+";")){
					var my_string = UM_startLog[str]
					break
				}
			}
			var splitString = my_string.split(";")
			//log(splitString)
			var myOldExpectedTime = splitString[7]
			//log(myOldExpectedTime)
			var newStringArray = [splitString[0],splitString[1],splitString[2],splitString[3],splitString[4],splitString[5],splitString[6],String(rtpObjects[objNr].expectedTimeSecs_ad),splitString[8],splitString[9],splitString[10],splitString[11],splitString[12],splitString[13],splitString[14]]
			//log(newStringArray[7])
			var adjustedString = newStringArray.join(";")
			
			UM_startLog = UM_startLog.map(function(x){ return x.replace(my_string,adjustedString) });
			UM_loopLogStrings = UM_loopLogStrings.map(function(x){ return x.replace(my_string,adjustedString) });
			adjustedStringsArray.push(my_matchID)
		}
